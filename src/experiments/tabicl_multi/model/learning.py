...
class ICLearning(nn.Module):
    ...
    def __init__(
        ...
        classes_per_feature: list[int] = None,  # NEW: for multi-output
    ):
        super().__init__()
        self.max_classes = max_classes
        self.norm_first = norm_first
        self.classes_per_feature = classes_per_feature  # NEW: store feature structure

        # NEW: Calculate total output dimension for multi-output
        if classes_per_feature is not None:
            self.total_output_dim = sum(classes_per_feature)
            self.multi_output = True
        else:
            self.total_output_dim = max_classes
        ...
        # MODIFIED: Decoder now outputs total_output_dim instead of max_classes
        self.decoder = nn.Sequential(nn.Linear(d_model, d_model * 2), nn.GELU(), nn.Linear(d_model * 2, self.total_output_dim))

        self.inference_mgr = InferenceManager(enc_name="tf_icl", out_dim=self.total_output_dim)
    ...
    def _predict_multi_output(  # ADDED: new method for (X_context, X_query) paradigm
        self,
        R_context: Tensor,  # ADDED: context for conditioning
        R_query: Tensor,  # ADDED: queries to reconstruct
        return_logits: bool = False,
        softmax_temperature: float = 0.9,
    ) -> Tensor:
        """Multi-output prediction: predict all features for query samples given context.

        Parameters
        ----------
        R_context : Tensor
            Context representations of shape (B, N_context, D)
        R_query : Tensor
            Query representations of shape (B, N_query, D)
        ...
        -------
        Tensor
            Predictions for all features in query samples of shape (B, N_query, total_output_dim)
        """
        R = torch.cat([R_context, R_query], dim=1)  # CHANGED: concat context and query instead of using y_train to split

        # CHANGED: skip y_encoder (no single-feature conditioning needed)
        src = self.tf_icl(R, attn_mask=R_context.shape[1])  # CHANGED: mask at context boundary instead of train_size
        if self.norm_first:
            src = self.ln(src)

        out = self.decoder(src)  # UNCHANGED: outputs (B, N_context + N_query, total_output_dim)

        out = out[:, R_context.shape[1]:, :]  # CHANGED: slice using context size instead of train_size

        if not return_logits:
            # ADDED: per-feature softmax (each feature sums to 1 independently, like Aerial)
            feature_probs = []  # ADDED: collect per-feature probabilities
            start_idx = 0  # ADDED: track position in output
            for num_classes in self.classes_per_feature:  # ADDED: iterate each feature
                end_idx = start_idx + num_classes  # ADDED: compute feature slice end
                feature_logits = out[:, :, start_idx:end_idx]  # ADDED: extract this feature's logits
                feature_prob = torch.softmax(feature_logits / softmax_temperature, dim=-1)  # ADDED: softmax per feature
                feature_probs.append(feature_prob)  # ADDED: collect result
                start_idx = end_idx  # ADDED: move to next feature
            out = torch.cat(feature_probs, dim=-1)  # ADDED: concat all per-feature probabilities
    ...
    def _predict_standard(
        ...
        # MODIFIED: For multi-output, keep all dimensions; for single-output, slice to num_classes
        if self.multi_output:
            out = out[:, train_size:, :]  # Keep all output dimensions
        else:
