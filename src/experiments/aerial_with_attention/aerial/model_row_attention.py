...
class AutoEncoder(nn.Module):
    def __init__(self, input_dimension, feature_count, layer_dims: list = None, embed_dim: int = 64, num_heads: int = 4,
        ...
        embed_dim = feature_count
        num_heads = feature_count
        ...
        self.row_attention = nn.MultiheadAttention(embed_dim, num_heads, dropout=attn_dropout, batch_first=True)
    ...
    def forward(self, x, feature_value_indices):
        ...
        # Project to (batch, feature_count, embed_dim)
        batch_size = y.size(0)
        ...
        # Self-aerial_with_attention across rows (transpose for row-wise aerial_with_attention)
        proj_transposed = proj.transpose(0, 1)  # (feature_count, batch, embed_dim)
        attn_out, _ = self.row_attention(proj_transposed, proj_transposed, proj_transposed)  # (feature_count, batch, embed_dim)
        attn_out = attn_out.transpose(0, 1)  # (batch, feature_count, embed_dim)

        # Collapse back
        attn_flat = attn_out.reshape(batch_size, -1)  # (batch, feature_count * embed_dim)
