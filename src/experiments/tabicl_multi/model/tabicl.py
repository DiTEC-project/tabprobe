...
class TabICL(nn.Module):
    ...
    def __init__(
        ...
        self.classes_per_feature = classes_per_feature  # NEW: store for later use
        ...
        # MODIFIED: Pass classes_per_feature to ICLearning
        self.icl_predictor = ICLearning(
            ...
            activation=activation,
            norm_first=norm_first,
            classes_per_feature=classes_per_feature,  # NEW: enable multi-output
        )
    ...
    def predict_multi_output(  # ADDED: new method for (X_context, X_query) paradigm
            self,
            X_context: Tensor,  # ADDED: noisy context table for conditioning
            X_query: Tensor,  # ADDED: query patterns to reconstruct
        ...
        ----------
        X_context : Tensor
            Context table of shape (B, N_context, H) for conditioning
        X_query : Tensor
            Query patterns of shape (B, N_query, H) to reconstruct
        ...
        -------
        Tensor
            Predictions for all features of shape (B, N_query, total_output_dim)
        """
        if inference_config is None:
            inference_config = InferenceConfig()

        # CHANGED: embed context and query separately (not concatenated in X)
        context_col_embed = self.col_embedder(  # ADDED: embed context table
            X_context,
            train_size=None,  # CHANGED: no train/test split needed
            mgr_config=inference_config.COL_CONFIG,
        )
        query_col_embed = self.col_embedder(  # ADDED: embed query patterns
            X_query,
            train_size=None,  # CHANGED: no train/test split needed
            mgr_config=inference_config.COL_CONFIG,
        )

        context_repr = self.row_interactor(context_col_embed,
                                           mgr_config=inference_config.ROW_CONFIG)  # ADDED: get context representations
        query_repr = self.row_interactor(query_col_embed,
                                         mgr_config=inference_config.ROW_CONFIG)  # ADDED: get query representations

        out = self.icl_predictor._predict_multi_output(  # ADDED: call new multi-output method instead of forward
            R_context=context_repr,  # ADDED: pass context representations
            R_query=query_repr,  # ADDED: pass query representations
            return_logits=return_logits,
            softmax_temperature=softmax_temperature,
        )
