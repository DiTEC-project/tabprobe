...
def compute_rule_stats(pyaerial_rules, dataset, feature_names):
    """Average support/confidence/Zhang's metric/interestingness/coverage over a list of
    PyAerial-format rules (as produced by convert_fpgrowth_rules_to_pyaerial_format),
    plus data coverage. Shared by fpgrowth_rule_learning and fpmax_rule_learning since
    both start from mlxtend association_rules output.
    """
...
if __name__ == "__main__":
    print("=" * 80)
    print("FP-Growth Rule Mining Experiments")
    print("=" * 80)
    print("\nNOTE: FP-Growth is deterministic. It runs once per dataset.")
    print("=" * 80)
    ...
    if len(all_results) > 0:
        ...
        with pd.ExcelWriter(output_filename, engine='openpyxl') as writer:
            ...
            params_df = pd.DataFrame([{
                ...
                'min_confidence': min_confidence,
                'min_support': min_support,
                'note': 'CPU-based FP-Growth using mlxtend (deterministic)'
            }])
