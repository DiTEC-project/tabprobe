...
Adapts TabICL for frequent itemset discovery using reconstruction-based approach.
"""
import time
import os
import numpy as np
...
if __name__ == "__main__":
    print("=" * 80)
    print("TabICL Frequent Itemset Mining")
    print("=" * 80)
    ...
    for dataset_info in datasets:
        ...
        for run_idx in range(n_runs):
            ...
            extracted_itemsets, stats, _, _ = tabicl_itemset_learning(
                dataset=dataset,
            ...
            if len(extracted_itemsets) > 0:
                save_itemsets(extracted_itemsets, stats, dataset_name, "tabicl", run_seed)
                result = {
    ...
    with pd.ExcelWriter(f"out/tabicl_itemsets_{timestamp}.xlsx", engine='openpyxl') as writer:
