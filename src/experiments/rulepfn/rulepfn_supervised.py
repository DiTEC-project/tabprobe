...
def train_on_tables(model, table, query, feature_value_indices, epochs=100, lr=5e-3,
                    noise_factor=0.5, device='cpu', context_ratio=0.8):
    """
    Train rulepfn model on ACTUAL TABLE ROWS (not test vectors).

    CORRECT TRAINING STRATEGY:
    - Split table into context_rows (80%) and target_rows (20%)
    - Learn to reconstruct REAL patterns (target rows) using context
    - Model learns: "what feature combinations exist in this table?"
    - At inference: test vectors matching real patterns → good reconstruction

    Args:
        model: TabularFoundationModel instance
        table: Table data (n_rows, n_features) - ACTUAL DATA
        query: Query data (test vectors) - ONLY used for inference, NOT training
        feature_value_indices: List of ranges for softmax application
        epochs: Number of training epochs
        lr: Learning rate
        noise_factor: Noise factor for denoising
        device: Device to train on
        context_ratio: Ratio of rows to use as context (default 0.8)

    Returns:
        trained_model: Trained model
        probability_matrix: Reconstruction probability matrix on query (for inference)
    """
    model = model.to(device)
    model.train()

    n_rows = table.shape[0]
    n_context = int(n_rows * context_ratio)

    # Split table into context and target
    indices = np.random.permutation(n_rows)
    context_indices = indices[:n_context]
    target_indices = indices[n_context:]

    context_rows = table[context_indices]
    target_rows = table[target_indices]

    # Convert to tensors
    context_tensor = torch.FloatTensor(context_rows).unsqueeze(0).to(device)
    target_tensor = torch.FloatTensor(target_rows).unsqueeze(0).to(device)

    # Create masks
    context_mask = torch.ones(1, len(context_indices), dtype=torch.bool, device=device)
    ...
    print(f"  - Training on {len(target_indices)} target rows with {len(context_indices)} context rows")

    for epoch in range(epochs):
        # Add noise to TARGET ROWS (real data, not test vectors!)
        noisy_targets = (target_tensor + torch.randn_like(target_tensor) * noise_factor).clamp(0, 1)

        # Forward pass: reconstruct REAL rows using context
        reconstructed, _ = model(context_tensor, noisy_targets, softmax_ranges, table_mask=context_mask)

        # Loss: how well can we reconstruct ACTUAL patterns?
        loss = criterion(reconstructed, target_tensor)
    ...
    # INFERENCE: Now test on query (test vectors) using ENTIRE table as context
    print(f"  - Inference on {query.shape[0]} test vectors using full table as context")
    model.eval()
    with torch.no_grad():
        full_table_tensor = torch.FloatTensor(table).unsqueeze(0).to(device)
        full_table_mask = torch.ones(1, n_rows, dtype=torch.bool, device=device)
        query_tensor = torch.FloatTensor(query).unsqueeze(0).to(device)

        probability_matrix, _ = model(full_table_tensor, query_tensor, softmax_ranges, table_mask=full_table_mask)

    return model, probability_matrix


def train_on_synthetic_tables_batch(model, table_batch, query_batch, feature_value_indices_batch,
                                    epochs=5, lr=5e-3, noise_factor=0.5, device='cpu',
                                    accumulation_steps=1, context_ratio=0.8):
    """
    Train rulepfn model on MILLIONS of tables with CORRECT strategy.

    TRAINING STRATEGY:
    - For each table: split into context (80%) and target (20%) rows
    - Learn to reconstruct REAL patterns from each table
    - Model learns general principles of associations across all tables
    - Foundation model: same weights work for ANY table at inference

    Args:
        model: TabularFoundationModel instance
        table_batch: List of tables, each (n_rows, n_features) with VARIABLE n_rows
        query_batch: List of query matrices (used ONLY for inference, not training)
        feature_value_indices_batch: List of feature_value_indices for each table
        epochs: Number of training epochs
        lr: Learning rate
        noise_factor: Noise factor for denoising
        device: Device to train on
        accumulation_steps: Number of steps for gradient accumulation
        context_ratio: Ratio of rows to use as context
    ...
    n_tables = len(table_batch)

    for epoch in range(epochs):
        epoch_loss = 0.0
        optimizer.zero_grad()

        for i, (table, query, fvi) in enumerate(zip(table_batch, query_batch, feature_value_indices_batch)):
            n_rows = table.shape[0]
            n_context = int(n_rows * context_ratio)
            ...
            context_tensor = torch.FloatTensor(context_rows).unsqueeze(0).to(device)
            target_tensor = torch.FloatTensor(target_rows).unsqueeze(0).to(device)
            softmax_ranges = [range(f['start'], f['end']) for f in fvi]

            # Create mask for context rows
            context_mask = torch.ones(1, len(context_indices), dtype=torch.bool, device=device)

            # Add noise to TARGET rows (real data!)
            noisy_targets = (target_tensor + torch.randn_like(target_tensor) * noise_factor).clamp(0, 1)

            # Forward pass: reconstruct REAL patterns using context
            reconstructed, _ = model(context_tensor, noisy_targets, softmax_ranges, table_mask=context_mask)

            # Loss: reconstruction of ACTUAL table patterns
            loss = criterion(reconstructed, target_tensor) / accumulation_steps
            loss.backward()

            epoch_loss += loss.item() * accumulation_steps

            # Update weights after accumulation steps
            if (i + 1) % accumulation_steps == 0 or (i + 1) == n_tables:
                optimizer.step()
                optimizer.zero_grad()
...
def extract_rules_from_reconstruction(prob_matrix, test_descriptions, feature_value_indices,
                                      table_data, ant_similarity=0.5, cons_similarity=0.8,
                                      feature_names=None, debug=False):
    """
    Extract association rules from reconstruction probability matrix with ACTUAL support and confidence.
    ...
    Args:
        ...
        feature_value_indices: List of dicts with 'start', 'end', 'feature' for each feature
        table_data: Original table data (n_rows, n_features) - NEEDED for calculating actual support/confidence
    ...
    Returns:
        association_rules: List of dicts with 'antecedents', 'consequent', 'support', 'confidence'
    """
    association_rules = []
    low_support_count = 0
    ...
    for i, antecedent_desc in enumerate(test_descriptions):
        ...
        for feat_info in feature_value_indices:
            ...
            if max_prob >= cons_similarity and global_idx not in candidate_antecedents:
                consequent_list.append((feat_info['feature'], max_class_idx, max_prob, global_idx))

        # Create rules for each consequent with ACTUAL support and confidence from table
        if consequent_list:
            # Calculate support for antecedent: how many rows have all antecedent features
            antecedent_mask = np.ones(n_rows, dtype=bool)
            for ant_idx in candidate_antecedents:
                antecedent_mask &= (table_data[:, ant_idx] == 1)

            antecedent_support = antecedent_mask.sum() / n_rows
            ...
            for cons_feat, cons_class, cons_prob, cons_idx in consequent_list:
                # Calculate support for rule (antecedent AND consequent)
                rule_mask = antecedent_mask & (table_data[:, cons_idx] == 1)
                rule_support = rule_mask.sum() / n_rows

                # Calculate confidence: P(consequent | antecedent) = support(A ∪ C) / support(A)
                confidence = rule_support / antecedent_support if antecedent_support > 0 else 0.0
                ...
                association_rules.append({
                    ...
                    'model_confidence': float(cons_prob),  # Model's predicted confidence
                    'support': float(rule_support),  # Actual support from table
                    'confidence': float(confidence)  # Actual confidence from table
                })
        else:
...
if __name__ == "__main__":
    ...
    # PyAerial similarity thresholds (Aerial defaults)
    ANT_SIMILARITY = 0.5
    CONS_SIMILARITY = 0.8
    ...
    # Prepare data for rulepfn model training
    print("\n" + "=" * 80)
    print("Preparing Datasets for Foundation Model")
    print("=" * 80)
    ...
    for dataset_name, encoded_data, classes_per_feat, feature_names in datasets:
        ...
        if encoded_data.shape[0] > MAX_ROWS:
            print(f"  - WARNING: Limiting to {MAX_ROWS} rows")
            encoded_data = encoded_data[:MAX_ROWS]

        # Generate test matrix for this dataset
        print(f"  - Generating test matrix with max {MAX_ANTECEDENTS} antecedents...")
        ...
        # Add to batch
        table_batch.append(encoded_data)
    ...
    # Step 1: Initialize ONE rulepfn model (works for ANY table)
    print("\n" + "=" * 80)
    print("STEP 1: Initialize Foundation Model")
    print("=" * 80)

    # Use maximum feature count across all datasets for model capacity
    max_feature_count = max(len(classes) for _, _, classes, _ in datasets)

    foundation_model = TabularFoundationModel(
        input_dim=INPUT_DIM,
        embed_dim=EMBED_DIM,
        ae_input_dim=INPUT_DIM,
        ae_feature_count=max_feature_count,
        max_rows=MAX_ROWS,
        num_heads=NUM_HEADS,
    ...
    print(f"  - Max rows supported: {MAX_ROWS}")
    print(f"  - Max features: {max_feature_count}")
    print(f"  - AutoEncoder dimensions: {foundation_model.autoencoder.dimensions}")

    # Step 2: Train ONE model on ALL tables
    print("\n" + "=" * 80)
    print("STEP 2: Training Foundation Model on All Tables")
    print("=" * 80)
    print(f"\nTraining on {len(table_batch)} tables with varying sizes...")
    print(f"  - Table 1: {table_batch[0].shape}")
    print(f"  - Table 2: {table_batch[1].shape}")

    trained_model = train_on_synthetic_tables_batch(
        model=foundation_model,
        table_batch=table_batch,
        query_batch=test_matrices,
        feature_value_indices_batch=feature_value_indices_list,
        epochs=200,
        lr=5e-3,
    ...
    print(f"  - SAME model will now extract rules from BOTH tables")
    print(f"\n{'-' * 80}")

    # Step 3: Extract rules from BOTH datasets using the SAME trained model
    print("\n" + "=" * 80)
    print("STEP 3: Extracting Rules Using Foundation Model")
    print("=" * 80)

    for idx, (dataset_info, test_matrix, test_descriptions, feature_value_indices) in enumerate(
            zip(datasets, test_matrices, test_descriptions_list, feature_value_indices_list)):
        ...
        # Extract rules using PyAerial logic with ACTUAL support and confidence
        ...
        association_rules = extract_rules_from_reconstruction(
            ...
            table_data=encoded_data,  # Pass actual table data for support/confidence calculation
        ...
        if len(association_rules) > 0:
            avg_support = np.mean([rule['support'] for rule in association_rules])
            avg_confidence = np.mean([rule['confidence'] for rule in association_rules])
            avg_model_confidence = np.mean([rule['model_confidence'] for rule in association_rules])

            print(f"\n  Average Rule Metrics:")
            print(f"    - Average Support: {avg_support:.4f}")
            print(f"    - Average Confidence: {avg_confidence:.4f}")
            print(f"    - Average Model Confidence: {avg_model_confidence:.4f}")
        else:
            print(f"\n  No rules extracted - cannot calculate averages")

        # Display first 10 rules with full metrics
        ...
        for rule_idx, rule in enumerate(association_rules[:10], 1):
            ...
            print(f"    Support: {rule['support']:.4f}, Confidence: {rule['confidence']:.4f}, "
                  f"Model Conf: {rule['model_confidence']:.4f}")

        print(f"\n{'-' * 80}")

    # Save rulepfn model for future use
    torch.save(trained_model.state_dict(), "tabular_foundation_model.pt")
    print("\n" + "=" * 80)
    print("Foundation Model Saved")
    print("=" * 80)
    print(f"  - Saved as 'tabular_foundation_model.pt'")
    print(f"  - This SINGLE model can extract rules from ANY table!")
